          ...
          >
            ...
            <div className="mt-6 grid grid-cols-2 gap-x-8 gap-y-5">
              {specs.map((spec) => (
                <div key={spec.label} className="flex items-start gap-3">
                  <spec.icon className="mt-0.5 h-4 w-4 text-accent" strokeWidth={1.5} />
                  <div>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              ...
              >
                ...
                {inquiryLoading ? '创建会话…' : '询价咨询'}
              </button>
              <button
                type="button"
                onClick={handleFavorite}
                className={cn(
                  'flex items-center justify-center gap-2 border px-8 py-3.5 text-xs uppercase tracking-[0.2em] transition-all duration-500',
                  isFavorited
