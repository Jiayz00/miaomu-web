  ...
  return (
    ...
    <main className="flex min-h-dvh flex-col items-center justify-center bg-paper px-6">
      <div className="max-w-md text-center">
        {/* 印章式错误标识 */}
        <div className="mx-auto mb-8 flex h-20 w-20 items-center justify-center border-2 border-gold/40" aria-hidden="true">
          <span className="font-serif text-3xl text-gold">误</span>
        </div>

        {/* role="alert" 让屏读器立即播报错误（WCAG 3.3.1 Error Identification / 4.1.3 Status Messages） */}
        <h1 className="font-serif text-3xl text-ink-text" role="alert">方寸有失</h1>
        ...
        {process.env.NODE_ENV === 'development' && (
          <details className="mt-6 rounded border border-ink-text-muted/15 bg-white p-3 text-left">
            <summary className="cursor-pointer text-xs text-ink-text-secondary">
              开发环境错误详情
            </summary>
            <pre className="mt-2 overflow-x-auto text-xs text-state-error">
              {error.message}
              {error.stack && `\n\n${error.stack}`}
            </pre>
          </details>
